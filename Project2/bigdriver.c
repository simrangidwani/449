...
// A test which ensures that next-fit works how it should.
void test_next_fit() {
	void* heap_at_start = start_test("test_next_fit");
	...
	if(should_be_c != c) {
		...
		// If we malloc 10 ints, next-fit should continue from there and
		// find the block at 'd', not at 'a'.

		int* should_be_d = make_array(10);

		if(should_be_d == a) {
			printf(RED("I think next fit started over at the beginning of the heap.\n"));
			my_free(should_be_d);
		} else if(should_be_d != d) {
			printf(RED("the 160-byte block was not reused.\n"));
			my_free(should_be_d);
		} else {
			// You correctly reused the block at 'd'. The heap should be like:
			// [F 40][U 16][F 80][U 16][U 120][U 16][U 160][U 16][F 200][U 16]
			// d should be the most-recently-allocated, so if we free it,
			// the most-recently-allocated should restart at the heap head.

			my_free(should_be_d);

			// and if we allocate a 10-int array... it should pick up a, not d or e.

			int* should_be_a = make_array(10);

			if(should_be_a == d || should_be_a == e) {
				printf(RED("When freeing a block, if it was last-allocated, reset last-allocated to the heap head.\n"));
			} else if(should_be_a != a) {
	...
	my_free(div4);
	my_free(div5);
	check_heap_size("test_next_fit", heap_at_start);
}
